...
def get_required_unity_paths
  ...
  raise "Unity not found at path '#{exe}' - set $UNITY_DIR (full path) or $UNITY_VERSION (loaded via hub) to customize" unless File.exist?(exe)
...
def unity(*cmd, force_free: true, no_graphics: true)
  ...
  cmd_prepend -= ["-force-free"] unless force_free
  cmd_prepend -= ["-nographics"] unless no_graphics
...
namespace :plugin do
  ...
  namespace :plugin do
    namespace :build do
      ...
      task :cocoa do
        Rake::Task["plugin:build:assets"].invoke
        next unless is_mac?
        build_type = "Release"
        FileUtils.mkdir_p cocoa_build_dir
        FileUtils.cp_r "bugsnag-cocoa/Bugsnag", cocoa_build_dir
