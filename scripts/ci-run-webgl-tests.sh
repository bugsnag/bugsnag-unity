...
bundle exec maze-runner --port=$((MAZE_RUNNER_PORT)) --document-server-port=$((MAZE_RUNNER_PORT + 1)) --farm=local --browser=firefox -e features/csharp/csharp_persistence.feature features/csharp
